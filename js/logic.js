		...
		success: function(response){
			if(response.dataC != null){
				...
				listventas.append(' \n\
					...
					<div class="col-md-3 mb-3">\n\
					  <label for="esta'+nextID+'" class="boxMinText">Estatus</label>\n\
					 <select class="form-control formsCRUD" id="esta'+nextID+'" required></select>\n\
					</div>\n\
					<div class="col-md-12 mb-3">\n\
				...
				');
				selectEstatus = $('#esta'+nextID+'');
				selectEstatus.html('');
				for (var i = response.estatus.length - 1; i >= 0; i--) {
					selectEstatus.append('<option value="'+response.estatus[i].est_codigo+'">'+response.estatus[i].est_nombre+'</option>');
				}
				$("#esta"+nextID+" option[value="+ response.dataC[0].fk_ven_estatus+"]").attr("selected",true);
			
	$('#mod'+nextID+'').on('click',function(e){
		e.preventDefault();
		var codigo =$(this).val();

		console.log ('esto', $('#esta'+nextID+'').children(":selected").val)
		// console.log ('este es el estatus', est);
		$.ajax({
